...
var rawIntroPointV3 = `introduction-point BAAGwWx24QG7AhSMqkcLkFdYdCID4+tFlBcZ/Kn+7AMgGOC0nmuoMP3c+cpu+iejBmfnw82ySyQJQP+g76QA4cwBEiYEhsDwAQAAAADQ7QDh0A8Buw==
...
func TestIntroductionPointV3Parse(t *testing.T) {
	ip := introductionPointV3Parse(rawIntroPointV3)
	encoded := ip.encode()
	assert.Equal(t, rawIntroPointV3, encoded)
}

func BenchmarkIntroductionPointV3Parse(b *testing.B) {
	b.ReportAllocs()
	ip := introductionPointV3Parse(rawIntroPointV3)
	for n := 0; n < b.N; n++ {
		_ = ip.encode()
...
func TestBlindedPubKey(t *testing.T) {
	...
	identityKey1 := gobpk.New(identityKey, false)
	...
	blindedKey := blindedPubkey(identityKey1, blindedNonce)
